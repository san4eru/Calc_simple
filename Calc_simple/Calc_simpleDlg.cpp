...
// Calc_simpleDlg.cpp: файл реализации
//
...
CCalcsimpleDlg::CCalcsimpleDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_CALC_SIMPLE_DIALOG, pParent)
	, e1(0)
	, r1(0)
	, e2(0)
	, e3(0)
...
BEGIN_MESSAGE_MAP(CCalcsimpleDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON1, &CCalcsimpleDlg::OnBnClickedButton1)
END_MESSAGE_MAP()


// Обработчики сообщений CCalcsimpleDlg
...
void CCalcsimpleDlg::OnBnClickedButton1()
{
	UpdateData();
	switch (r1) 
	{
		...
		case 3:
			if (e2 == 0)
				MessageBox(L"Деленить на 0 нельзя!");
			else
				e3 = e1 / e2; 
			break;
	}
